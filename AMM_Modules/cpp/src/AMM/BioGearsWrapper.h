...
class BioGearsWrapper {
	std::unique_ptr<PhysiologyEngine> bg;
	std::string _patFile = "";
	std::string _condFile = "";
	std::string _stateFile = "";

public:
	BioGearsWrapper(std::string logFile);

	virtual ~BioGearsWrapper();

	void SetPatientFile(std::string patFile);
	void SetConditionsFile(std::string condFile);
	void SetStateFile(std::string stateFile);

	bool InitializeEngine();

	bool LoadConditionsAndPatient(std::string condFile, std::string patFile);
	...
	AMM::Physiology::Node* GetNodePath(const std::string& nodePath);
	void Status();
